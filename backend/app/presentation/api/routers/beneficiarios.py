...
def listar_tipos_espina(current_user: Annotated[dict, Depends(get_current_user)] = None):
    return service.listar_tipos_espina(current_user)

@router.get('/stats')
def stats_beneficiarios(current_user: Annotated[dict, Depends(get_current_user)] = None):
    return service.stats_beneficiarios(current_user)

@router.get('/stats/dashboard')
def dashboard_stats(current_user: Annotated[dict, Depends(get_current_user)] = None):
...
@router.get('/mapa')
def mapa_beneficiarios(current_user: Annotated[dict, Depends(get_current_user)] = None):
    return service.mapa_beneficiarios(current_user)

@router.get('/{folio}', response_model=BeneficiarioResponse)
def obtener_beneficiario(folio: str, current_user: Annotated[dict, Depends(get_current_user)] = None):
    return service.obtener_beneficiario(folio, current_user)

@router.post('', status_code=201, response_model=BeneficiarioResponse)
def crear_beneficiario(data: BeneficiarioCreate, current_user: Annotated[dict, Depends(require_role('ADMINISTRADOR', 'RECEPCIONISTA'))] = None):
    return service.crear_beneficiario(data, current_user)

@router.put('/{folio}', response_model=BeneficiarioResponse)
def actualizar_beneficiario(folio: str, data: BeneficiarioCreate, current_user: Annotated[dict, Depends(require_role('ADMINISTRADOR', 'RECEPCIONISTA'))] = None):
    return service.actualizar_beneficiario(folio, data, current_user)

@router.delete('/{folio}', status_code=200)
def eliminar_beneficiario(folio: str, current_user: Annotated[dict, Depends(require_role('ADMINISTRADOR'))] = None):
...
def renovar_membresia(folio: str, data: RenovarMembresiaCreate, current_user: Annotated[dict, Depends(require_role('ADMINISTRADOR', 'RECEPCIONISTA'))] = None):
