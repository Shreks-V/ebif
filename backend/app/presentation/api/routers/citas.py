...
@router.get('/stats')
def citas_stats(current_user: Annotated[dict, Depends(get_current_user)] = None) -> dict:
    return service.citas_stats(current_user)

@router.get('/hoy')
def citas_hoy(current_user: Annotated[dict, Depends(get_current_user)] = None) -> dict:
...
@router.get('/{id_cita}')
def obtener_cita(id_cita: int, current_user: Annotated[dict, Depends(get_current_user)] = None) -> dict:
    return service.obtener_cita(id_cita, current_user)

@router.post('', status_code=201)
def crear_cita(data: CitaCreate, current_user: Annotated[dict, Depends(require_role('ADMINISTRADOR', 'RECEPCIONISTA'))] = None) -> dict:
    return service.crear_cita(data, current_user)

@router.put('/{id_cita}')
def actualizar_cita(id_cita: int, data: CitaCreate, current_user: Annotated[dict, Depends(require_role('ADMINISTRADOR', 'RECEPCIONISTA'))] = None) -> dict:
...
def cancelar_cita(id_cita: int, current_user: Annotated[dict, Depends(require_role('ADMINISTRADOR', 'RECEPCIONISTA'))] = None) -> dict:
    return service.cancelar_cita(id_cita, current_user)

@router.delete('/{id_cita}')
def eliminar_cita(id_cita: int, current_user: Annotated[dict, Depends(require_role('ADMINISTRADOR'))] = None) -> dict:
