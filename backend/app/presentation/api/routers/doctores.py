...
@router.get('/hoy')
def doctor_del_dia(current_user: Annotated[dict, Depends(get_current_user)] = None):
...
def obtener_doctor(id_doctor: int, current_user: Annotated[dict, Depends(get_current_user)] = None):
    return service.obtener_doctor(id_doctor, current_user)

@router.post('', status_code=201)
def crear_doctor(data: DoctorCreate, current_user: Annotated[dict, Depends(require_role('ADMINISTRADOR'))] = None):
    return service.crear_doctor(data, current_user)

@router.put('/{id_doctor}')
def actualizar_doctor(id_doctor: int, data: DoctorCreate, current_user: Annotated[dict, Depends(require_role('ADMINISTRADOR'))] = None):
    return service.actualizar_doctor(id_doctor, data, current_user)

@router.delete('/{id_doctor}')
def desactivar_doctor(id_doctor: int, current_user: Annotated[dict, Depends(require_role('ADMINISTRADOR'))] = None):
...
def crear_disponibilidad(id_doctor: int, data: DisponibilidadCreate, current_user: Annotated[dict, Depends(get_current_user)] = None):
    return service.crear_disponibilidad(id_doctor, data, current_user)

@router.delete('/{id_doctor}/disponibilidad/{id_disponibilidad}')
def eliminar_disponibilidad(id_doctor: int, id_disponibilidad: int, current_user: Annotated[dict, Depends(get_current_user)] = None):
    return service.eliminar_disponibilidad(id_doctor, id_disponibilidad, current_user)

@router.get('/{id_doctor}/servicios')
def obtener_servicios_doctor(id_doctor: int, current_user: Annotated[dict, Depends(get_current_user)] = None):
    return service.obtener_servicios_doctor(id_doctor, current_user)

@router.get('/{id_doctor}/disponibilidad-especial')
def listar_disponibilidad_especial(id_doctor: int, current_user: Annotated[dict, Depends(get_current_user)] = None):
...
def crear_disponibilidad_especial(id_doctor: int, data: DisponibilidadEspecialCreate, current_user: Annotated[dict, Depends(get_current_user)] = None):
...
def eliminar_disponibilidad_especial(id_doctor: int, id_disp_especial: int, current_user: Annotated[dict, Depends(get_current_user)] = None):
