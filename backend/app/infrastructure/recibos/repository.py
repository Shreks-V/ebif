...
from app.domain.recibos.constants import METODO_EFECTIVO, METODO_TARJETA, METODO_TRANSFERENCIA

logger = logging.getLogger(__name__)

_SQL_STATS_METODOS_PAGO = f"""
    SELECT NVL(SUM(CASE WHEN UPPER(mp.NOMBRE) = '{METODO_EFECTIVO}'      THEN vmp.MONTO ELSE 0 END), 0) AS monto_efectivo,
           NVL(SUM(CASE WHEN UPPER(mp.NOMBRE) = '{METODO_TARJETA}'       THEN vmp.MONTO ELSE 0 END), 0) AS monto_tarjeta,
           NVL(SUM(CASE WHEN UPPER(mp.NOMBRE) = '{METODO_TRANSFERENCIA}' THEN vmp.MONTO ELSE 0 END), 0) AS monto_transferencia
...
def _stats_ventas(_current_user: CurrentUser | None = None):
    ...
    with get_db() as conn:
        ...
        cur.execute(_SQL_STATS_METODOS_PAGO)
...
def _listar_ventas(fecha_inicio: str | None=None, fecha_fin: str | None=None, id_paciente: int | None=None, search: str | None=None, _current_user: CurrentUser | None = None, limit: int=100, offset: int=0, solo_adeudos: bool=False):
...
def _exentar_venta(id_venta: int, nota: str | None=None, current_user: CurrentUser | None = None):
...
def _cancelar_venta(id_venta: int, motivo: str | None=None, current_user: CurrentUser | None = None):
    """Cancelar una venta."""
    with get_db() as conn:
        cur = conn.cursor()
