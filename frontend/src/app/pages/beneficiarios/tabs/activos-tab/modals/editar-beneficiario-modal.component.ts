...
export class EditarBeneficiarioModalComponent implements OnChanges {
  ...
  get editFechaDia(): string { return this.editFormData?.fecha_nacimiento?.split('-')[2] ?? ''; }
  set editFechaDia(v: string) { this._setEditFecha(undefined, undefined, v); }
  get editFechaMes(): string { return this.editFormData?.fecha_nacimiento?.split('-')[1] ?? ''; }
  set editFechaMes(v: string) { this._setEditFecha(undefined, v); }
  get editFechaAnio(): string { return this.editFormData?.fecha_nacimiento?.split('-')[0] ?? ''; }
  set editFechaAnio(v: string) { this._setEditFecha(v); }

  private _setEditFecha(anio?: string, mes?: string, dia?: string): void {
    if (!this.editFormData) return;
    const [a, m, d] = this.editFormData.fecha_nacimiento?.split('-') ?? ['', '', ''];
    const na = anio ?? a; const nm = mes ?? m; const nd = dia ?? d;
    if (na && nm && nd) this.editFormData.fecha_nacimiento = `${na}-${nm}-${nd}`;
  }
  // ──────────────────────────────────────────────────────────────────────────
