/**
 * E2E tests: Public pre-registro form at /preregistro.
 *
 ...
 *
 * Covers:
 *  - Page loads at /preregistro (no login needed)
...
test.describe('Pre-registro – Carga de página', () => {
  ...
  test('pagina_carga_sin_autenticacion – /preregistro loads without login', async ({ page }) => {
    await setupPreregistroMocks(page);

    await page.goto('/preregistro');
    await page.waitForTimeout(2000);

    // Must NOT redirect to login
    const url = page.url();
    expect(url).toContain('/preregistro');
