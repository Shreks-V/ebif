...
test.describe('Paginación – Beneficiarios', () => {
  test('paginacion_con_datos_grandes – Siguiente / Anterior buttons present with 25 rows', async ({ page }) => {
    ...
    // mockAll FIRST so our large-mock route (registered after) has LIFO priority
    await api.mockAll();

    // Register large dataset AFTER mockAll — Playwright executes routes in LIFO order,
    // so this handler runs first and intercepts beneficiarios list calls.
    ...
    await page.route('**/api/beneficiarios**', (route) => {
      ...
      if (url.includes('/stats') || url.includes('/dashboard') || url.includes('/historial') ||
        ...
        route.fallback(); // let mockAll handler deal with stats
        return;
      }
